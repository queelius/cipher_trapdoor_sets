...
\begin{document}
\section{The $k$-disjoint hash set ($k$-DHS)}
\label{sec:shs}
The $k$-disjoint hash set ($k$-DHS), like the Bloom filter, is a model of \emph{Bernoulli sets} that obtains optimality with respect to \emph{space complexity} (or, relatedly, entropy).
It is more generally an example of the $k$-disjoint hash map ($k$-DHM), as a model of Bernoulli maps.

Essentially, the $k$-hash set bins the elements in some \emph{guard} set of size $m$ (typically the objective set) to $m/k$ bins with $k$ elements per bin.
Then, the elements in each bin is hashed to a value that encodes \emph{true} or \emph{false} depending on its membership status.

There are two \emph{special} cases to consider.

We denote the $m$-disjoint hash set the \emph{singular hash set} (SHS).
It is optimal with respect to space complexity and entropy, i.e., prior to performing the hashes the value of the data structure is a random bit string with an uncertain length whose expectation obtains the information-theoretic lower-bound.

In the $1$-disjoint hash set, we may directly map the value of that bin to the membership status of the single element in that bin, in which case we denote it the \emph{Perfect hash filter}.


We pay particular attention to these two special cases.



First, we describe the general algorithm.
Then, we show how it may be parameterized to generate up to a $4$-th order model.
...
\begin{definition}
    ...
    \end{equation}
    where $r$ is the maximum bits (which may cause a rate distortion) and
    \begin{equation}
        \begin{split}
            t			& \coloneqq 2^{r+1}-1\,,\\
            \hash_1(x,n)& \coloneqq \trunc(\hash(x' \cat n'),1)\,,\\	
            \Fun{g}(k) 	& \coloneqq \sum_{{x \in \Set{X}}}\left[\hash_1(x,k) \neq \SetIndicator{\Set{A}}(x)\right]\,,\\
...
\begin{definition}
The \emph{membership} relation
\begin{equation}
    \SetContains \colon \Set{X} \times \FoSHS_{\Set{X}} \mapsto \BitSet
\end{equation}
is defined as
\begin{equation}
    \Tuple{n',f} \SetContains x \coloneqq \trunc(\hash(x' \cat n'),1)\,.
\end{equation}
\end{definition}

The pair $\left(\FoSHS_{\Set{X}}, \SetContains\right)$ models a first-order random approximate algebraic structure $\left(\PS{\Set{X}},\SetContains\right)$.
...
\end{theorem}
\begin{proof}
In order for the value $\FoShs_{\Set{X}}(\Set{A},r)$ to be a first-order random approximate set with a distribution
given by $\ASetFO{A}[\epsilon]$, by \cref{def:approx_set} it must satisfy the \emph{a priori}
\begin{equation}
...
We can partition the universal set $\Set{U}$ into two blocks, a positive and negative block with respect to some objective set $\Set{A}$.

% more generally, we can partition the universal set however we wish and apply the above algorithm to some of those blocks to get a higher-order model.
% how do we decide which algo to use for a block? could be pre-agreed, could be a simple algorithm.
...
\begin{definition}
    ...
    The \emph{data type} for the \emph{cryptographic} second-order singular hash set is defined as $\SoSHS_{\Set{X}} \coloneqq \BitSet^* \times \NatSet \times \NatSet$ with a value constructor $\SoShs_{\Set{X}} \colon \PS{\Set{X}} \times \NatSet \mapsto \SoSHS_{\Set{X}}$ defined as
    \begin{equation}
...
The \emph{membership} relation
\begin{equation}
\SetContains \colon \Set{X} \times \SoSHS_{\Set{X}} \mapsto \BitSet
\end{equation}
is defined as
\begin{equation}
\Tuple{n',f_n,k} \SetContains x \coloneqq \trunc(\hash(x' \cat n'),k) \in E
\end{equation}
where
\begin{equation}
E \coloneqq \text{prefix-free encodings for $\True$}\,.
\end{equation}
